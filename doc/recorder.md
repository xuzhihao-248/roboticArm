...
### 2026/07/18  xuzhihao-248

**类型**: 代码

**内容**: JH_FOC_42V3 成品驱动器 CAN 总线 CLI 控制工具开发完成

*换厂家驱动板后 CAN 通信恢复正常，开发了三层模块化 CLI 工具并通过实机测试。*

**工具文件**: `my/tools/`

| 文件 | 职责 | 依赖 |
|------|------|------|
| `jh_foc_protocol.py` | CAN 协议常量、命令构建、响应解析 | stdlib |
| `jh_foc_driver.py` | 单电机/多电机控制类、CAN 扫描连接 | python-can |
| `motor_cli.py` | 交互终端（cmd.Cmd）+ 关节仿真测试套件 | 上述两个模块 |

**硬件参数**:
- 适配器: PCAN-USB (`PCAN_USBBUS1`)
- 波特率: **500 kbps**
- CAN ID: Joint 1-6 → **0x200-0x205**

#### 一、CAN 协议速查（JH_FOC_42V3）

**模式切换** (3字节): `[0x23, mode, 0x00]` — 切换后建议等待 300ms
...
| 命令 | 帧格式 (byte 0-7) | 说明 |
|------|-------------------|------|
| 绝对位置 | `0x57, sub, curr_H, curr_L, deg[31:24], deg[23:16], deg[15:8], deg[7:0]` | deg = 目标角度×100 (int32 BE) |
| 相对位置 | `0x55, sub, curr_H, curr_L, deg[31:24], deg[23:16], deg[15:8], deg[7:0]` | deg = 偏移量×100 (int32 BE) |
| 速度 | `0x33, 0x00, accel_H, accel_L, curr_H, curr_L, rpm_H, rpm_L` | rpm 为 int16 (有符号) |
| 角度模式 | `0x66, sub, curr_H, curr_L, 0, 0, ang_H, ang_L` | ang = 0-360°×100 (int16 BE) |
| 同步触发 | `0x99, 0x9A, 0, 0, 0, 0, 0, 0` | 广播到 ID 0x200 |
| 紧急停止 | `0x77, 0x78, 0, 0, 0, 0, 0, 0` | -- |
| 编码器校准 | `0x11, 0x22, 0, 0, 0, 0, 0, 0` | 电机正反转各一圈 |

**sub 字节**: `0xAA` = 缓存等待同步触发, `0x00` = 立即执行

**查询命令** (8字节) → 响应在 bytes[4:7] 为 int32 BE:

| 查询 | 命令字节 | 响应解析 |
|------|----------|----------|
| 模式 | `0xC9, 0xA9, 0,0,0,0,0,0` | byte[7] = 模式码 |
| 位置 | `0xC3, 0xA3, 0,0,0,0,0,0` | bytes[4:7] = 角度×100 |
| 速度 | `0xC0, 0xA0, 0,0,0,0,0,0` | bytes[4:7] = rpm×100 |

#### 二、CLI 使用

```bash
# 启动交互模式
cd my/tools && source .venv/Scripts/activate
python motor_cli.py -i pcan -c PCAN_USBBUS1

# 扫描并退出
python motor_cli.py --scan

# 单命令模式
python motor_cli.py -j 1 --cmd "abs 45"
python motor_cli.py -j 1 --cmd "calibrate"
```
...
|------|------|------|
| `scan` | 扫描 CAN 总线电机 | -- |
| `sel 1` | 选择当前关节 | `sel 1-3`, `sel all` |
| `posmode` | 切换到位置模式 | -- |
| `abs <deg>` | 绝对位置运动 | `abs 45.0` |
| `rel <deg>` | 相对位置运动 | `rel -30.0` |
| `speed <rpm>` | 设置速度(SPEED模式) | `speed 300` |
| `pos / spd / mode` | 查询状态 | -- |
| `state` | 查询完整状态 | -- |
| `calibrate` | 编码器校准(交互模式需确认) | -- |
| `estop` | 紧急停止所有电机 | -- |
| `idle` | 切换回 IDLE | -- |
| `test-ptp` | 点对点位置测试 | -- |
| `test-swing` | 往复摆动测试 | -- |
...
**快捷语法**: `3 abs 90` = 选择关节3并移动到90°

#### 三、实机测试结果

| 测试项 | 结果 | 精度 |
|--------|------|------|
| CAN 扫描 | PCAN_USBBUS1 @ 500kbps, 检测到 0x200 | -- |
| 编码器校准 | 校准成功, 6s 后恢复响应 | -- |
| PTP 点对点测试 | 0→+45→-45→0 通过 | 误差 <0.03° |
| 摆动测试 | 5 周期 ±30° 往复通过 | 漂移 0.00° |
| 位置绝对模式 | 44.97° / -45.00° | 误差 <0.03° |

#### 四、已知问题

| 问题 | 现象 | 应对 |
|------|------|------|
| 模式查询不可靠 | `get_mode()` 有时返回 IDLE/N/A | 模式切换已发送，运动命令正常执行 |
| 位置查询运动中返回 None | 驱动板 busy 时无响应 | 等待运动完成后查询 |
| 速度模式不稳定 | 切换到 SPEED 后电机不转 | 优先使用位置模式（精度已满足机械臂需求） |
| 校准后需等待 | 校准后 3-6s 驱动板才恢复响应 | 代码已内置 6s 等待 + 重试 |